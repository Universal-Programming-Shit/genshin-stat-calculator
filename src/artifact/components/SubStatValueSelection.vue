...
const availableRollsList = computed<number[][]>(() =>
  getElem(possibleRolls, props.rollAmount)
);
const selectedIndex = ref<number>(
  props.modelValue ? availableRollsList.value.indexOf(props.modelValue) : 0
);
...
watch(
  ...
  () => {
    if (props.modelValue === undefined) {
      ...
      props.rollAmount !== props.modelValue.length ||
      props.rollAmount !== props.modelValue.filter((value) => value).length
    ) {
