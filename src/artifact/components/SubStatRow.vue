...
const availableRolls = computed(() => {
  return removeDuplicates(possibleValues(rolls.value).sort())
...
function removeDuplicates(arr: number[][]) {
  return arr.filter((value: number[], index) => {
    return index === arr.findIndex((obj: number[]) => {
      return Math.fround(obj.reduce(add, 0)) === Math.fround(value.reduce(add, 0));
    });
  });
}

function possibleValues(n): number[][] {
  if (n <= 0) {
    return [[0]];
  } else if (n === 1) {
    ...
    return substatRolls.flatMap(value => values.map(value1 => [value, ...value1]));
  }
}
